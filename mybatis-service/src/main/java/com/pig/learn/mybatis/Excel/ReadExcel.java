...
public class ReadExcel<T> {
    private static final Logger ERROR_LOGGER = LoggerFactory.getLogger(ReadExcel.class);
                ...
                if (row != null) {
                    ...
                    for (int j = 0; j < row.getLastCellNum() && j < fieldNameList.size(); j++) {
                        ...
                        try {
                            ...
                            ERROR_LOGGER.error("参数异常!", e);
                        } catch (InvocationTargetException e) {
                            ERROR_LOGGER.error("setter方法产生异常!", e);
                        } catch (NoSuchMethodException e) {
                            ERROR_LOGGER.error("未找到 " + "set" + StringUtils.capitalize(fieldName) + " 方法!", e);
                        } catch (Exception e) {
                            ERROR_LOGGER.info(field.getName() + "数据错误!");
                        }
                    }
                    if (entity != null) {
                        objects.add(entity);
